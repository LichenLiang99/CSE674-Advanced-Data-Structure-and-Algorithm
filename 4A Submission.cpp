...
void Path(vector<list<pair<int, int>>>& graph, vector<Node>& table, vector<int>& heap){
	...
	while (heap.empty() == false){
		...
		for (iter = graph[minIndex].begin(); iter != graph[minIndex].end(); iter++){
			...
			//if the destination has not been visited and the cost is greater than the total of current cost to the destination
			//and the cost to that source node, then update the cost to the destination node and 'from'.
			if (table[Destination].checked == false && table[Destination].cost > table[minIndex].cost + cost){
				table[Destination].cost = table[minIndex].cost + cost;
	...
	for (int i = 0; i < table.size(); i++){
		...
		cout << "cost from " << Source << " to " << i << " is : " << table[i].cost;

		//if there is a path, print the previous,
		if (table[i].cost != 9999){
			cout << "  from node is " << table[i].from << "  Path: ";
			int node = i;
			cout << node;

			//keep printing until it reach the source node
			while (node != Source){
				node = table[node].from;
				cout << "<-" << node;
			}
		}
		cout << endl;
